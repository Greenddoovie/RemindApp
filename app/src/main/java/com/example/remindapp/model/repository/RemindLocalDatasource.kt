...
class RemindLocalDatasource(private val remindDatabase: RemindDatabase) : IRemindRepo {
    override fun getAll(): List<Remind> = remindDatabase.remindDao().getAll()

    override fun insert(remind: Remind) = remindDatabase.remindDao().insert(remind)

    override fun update(remind: Remind) = remindDatabase.remindDao().update(remind)
    ...
    override fun getRemind(idx: Int): Remind = remindDatabase.remindDao().getRemind(idx)

    override fun getRemind(title: String, hour: Int, minute: Int, uri: String): Remind =
        remindDatabase.remindDao().getRemind(title, hour, minute, uri)
